...
download.file("https://www.michigan.gov/documents/coronavirus/Cases_and_Deaths_by_County_and_Date_2020-09-17_702628_7.xlsx", destfile="./rawData/Cases_by_County_and_Date.xlsx")
...
```{r Detroit_plot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=10, fig.cap="**Figure 3:** A comparison of COVID-19 incidence between metropolitan Detroit (red) and the rest of Michigan (blue). The total state numbers are shown in green. Seven-day moving averages of the data are shown as dotted lines. The veritcal lines denote holidays: The 4th of July and Labor Day on September 7th."}
...
dates <- c("08/24-8/27", "08/28-09/03", "09/04-09/10","9/11-9/17")
...
ferris <- c(4,19,34,45)
mecosta <- c(week1,week2,week3,week4)
...
As of September 17, Ferris State accounts for the vast majority of all reported cases in Mecosta county. There does not yet appear to be a community-associated outbreak at this point. The Mecosta county numbers often take several days to get processed and reported. We will see if the numbers now begin to decline or if COVID-19 moves from campus out into the general population.

```{r ferris, echo=FALSE, comment=FALSE, error=FALSE, warning=FALSE, message=FALSE, fig.width=12, fig.height=7, fig.cap="**Figure 8:** Total COVID-19 cases at Ferris State University since the beginning of the fall semester."}

ferris_days <- c(4,12,19,23,24,25,26)
ferris_cases <- c(4,19,53,82,84,88,98)
...
ggplot(ferris_total, aes(x=Days, y=Cases)) +
  ...
  stat_smooth(method="loess", se=FALSE,  color="firebrick", fill="firebrick", alpha=0.3) +
...
* After adjusting for county population, metropolitan Detroit counties have about 3-times more cases and 7-times more deaths than other Michigan countiues.
