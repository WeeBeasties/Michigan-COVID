...
download.file("https://www.michigan.gov/documents/coronavirus/Cases_and_Deaths_by_County_and_Date_2020-09-30_703900_7.xlsx", destfile="./rawData/Cases_by_County_and_Date.xlsx")
...
```{r by_county, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=12, fig.cap="**Figure 3:** Normalized COVID-19 death rates by county (deaths per 100,000 people). The vertical lines indicate the various mortality rates for Michigan (including the overall mortality rate for COVID-19). This average is heavily biases by the five counties with extremely high mortalities."}

stateDeaths <- sum(metroDataCount$Deaths)/sum(metroDataCount$pop2018)*100000
annotation <- tibble(x = c(10,10,10,10,10,10),
                     y = c(9.7, 14.5, 44.2, stateDeaths, 161, 195),
                     label = c("Traffic accidents", "Influenza/Pneumonia","All accidents", "COVID-19", "Cancers", "Heart disease"))
...
ggplot(metroDataNorm, aes(x=reorder(County, +Deaths), y=Deaths)) + 
  ...
  xlab("County") +
  ylim(0,200) +
  geom_hline(yintercept = c(9.7, 14.5, 44.2, stateDeaths, 161, 195), linetype=1) +
  geom_text(data=annotation, aes( x=x, y=y, label=label), color="black", size=5 , angle=90, fontface="bold", nudge_y = 2) +
  coord_flip() +
  theme_bw()
```
...
ferris_days <- c(4,11,18,22,23,24,25,26,29,30,31,32,33,36,37,38)
ferris_cases <- c(4,19,53,82,84,88,98,105,113,114,116,116,121,131,136,138)
