...
download.file("https://www.michigan.gov/documents/coronavirus/Cases_and_Deaths_by_County_and_Date_2020-08-27_700645_7.xlsx", destfile="./rawData/Cases_by_County_and_Date.xlsx")
...
michiganTotalData$Metro <- ifelse(michiganTotalData$County %in% metroCounty, "Yes", "No")
...
metroDetroit <- metroDetroit[1:168, ]
...
metroDataCount <- michiganTotalData %>% 
  group_by(County) %>% 
  summarize(Cases = max(Cases.Cumulative), Deaths = max(Deaths.Cumulative), Metro = Metro)
metroDataCount <- unique(metroDataCount)

metroDataNorm <- michiganTotalData %>% 
  group_by(County) %>% 
  summarize(Cases = max(Cases.Cumulative)/pop2018*100000, Deaths = max(Deaths.Cumulative)/pop2018*100000, Metro = Metro)
metroDataNorm <- unique(metroDataNorm)
```

## Abstract  
This is a pseudo-scientific analysis of some COVID-19 trends in Michigan that interest me. I strongly suspect that the vast majority of cases and adverse outcomes are concentrated in the Detroit area. Our response as a state, however, has not really been defined geographically. This is probably not the best way to steward our limited resources (note for future microbial pandemics).
...
```{r state_map1, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=14, fig.cap="**Figure 1:** State-wide distribution of COVID-19. Panel A shows total cases, while panel B shows total deaths."}
...
```{r state_map2, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=14, fig.cap="**Figure 2:** State-wide distribution of COVID-19 normalized for county population. Panel A shows total cases per 100,000 people, while panel B shows total deaths per 100,000 people."}
...
my_t.test <- t.test(Deaths~Metro, data=metroDataNorm)
```
...
```{r Detroit_plot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=10, fig.cap="**Figure 3:** A comparison of COVID-19 incidence between metropolitan Detroit (red) and the rest of Michigan (blue). Five-day moving averages of the data are shown as dotted lines."}
plotA <- ggplot(compareDetroit, aes(x=Date, y=Cases, color=County)) +
  ...
  geom_ma(ma_fun = EMA, n = 5, wilder = TRUE, linetype = 3, size = 1.25) +
  theme_bw() +
...
## Comparing Mecosta (Ferris) and Isabella (Central Michigan) county cases  

Finally, I am also interested in comparing how Ferris State University does in controlling COVID-19 compared to Central Michigan University. Figure 4 compares the total number of COVID-19 cases between Mecosta and Isabella counties.

```{r county_plot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=7, fig.cap="**Figure 4:** A comparison of the number of total cases in Mecosta (red) and Isabella (blue) counties. The bars indicate total daily cases and the lines are five-day moving averages."}
counties <- michiganTotalData %>% 
  filter(County == "isabella" | County == "mecosta" )
ggplot(counties, aes(x=Date, y=Cases, color=County)) +
  scale_colour_manual(values=c('mecosta'="firebrick", 'isabella'="dodgerblue")) +
  geom_bar(stat = "identity") +
  geom_ma(ma_fun = EMA, n = 5, wilder = TRUE, linetype = 1, size = 2) +
  ylim(0,30) +
  theme_bw() +
  theme(legend.position="none")
```

CMU students went back to school earlier than Ferris students. It appears that this influx of students (and increased testing on campus) resulted in a spike of cases. It will be interesting to see how big of a spike Mecosta county experiences over the next few weeks.
